read_full_data <- function() {
  ...
  Full_Date <- strptime(Full_Date, format = "%d/%m/%Y %H:%M:%S")
  consumption <- cbind(Full_Date, raw_data)
  
  consumption
}

get_filtered_data <- function(consumption_data) {
  # Filter out records in range of 2007-02-01 and 2007-02-02:
  first_feb <- strptime("2007-02-01 00:00:00", format = "%Y-%m-%d %H:%M:%S")
  second_feb <- strptime("2007-02-02 23:59:59", format = "%Y-%m-%d %H:%M:%S")
  filtered_by_date <- subset(consumption_data, Full_Date >= first_feb & Full_Date <= second_feb)
